...
- **`npm run seo:generate`** (`scripts/generateSeoAssets.js`) reads the catalog and writes `public/sitemap.xml`, `public/robots.txt` and `react-snap-routes.json`. It also runs automatically after `npm run build` (see `postbuild` in package.json). Safe to run any time; it only touches those three files.
