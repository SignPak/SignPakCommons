...
let server, memory, base, admin, learner, other
const asJson = (res) => res.body
const learnerData = { firstName: 'Sara', surname: 'Ahmed', email: 'Sara@Example.com', password: 'password1', confirmPassword: 'password1' }
...
before(async () => {
  ...
  learner = new Client(base)
  assert.equal((await learner.post('/auth/signup', learnerData)).status, 201)
...
describe('auth', () => {
  test('signup validates every field', async () => {
    ...
    const mismatch = await new Client(base).post('/auth/signup', { ...learnerData, email: 'x@example.com', confirmPassword: 'different' })
    assert.equal(mismatch.body.error.fields.confirmPassword, 'Passwords do not match.')
    const short = await new Client(base).post('/auth/signup', { ...learnerData, email: 'x@example.com', password: 'short', confirmPassword: undefined })
  ...
  test('duplicate emails conflict, case-insensitively', async () => {
    const res = await new Client(base).post('/auth/signup', { ...learnerData, email: 'SARA@example.com' })
  ...
  test('admin-only routes reject learners and visitors', async () => {
...
describe('profile connections', () => {
  test('normalises, validates, and disconnects', async () => {
    const ok = await learner.patch('/users/me', { connections: { github: 'https://github.com/octocat', linkedin: 'linkedin.com/in/sara-ahmed' } })
    ...
    const bad = await learner.patch('/users/me', { connections: { github: 'not a user!!' } })
    assert.equal(bad.status, 422)
    assert.ok(bad.body.error.fields['connections.github'])
    const off = await learner.patch('/users/me', { connections: { github: null } })
...
let daily, work, lesson1, lesson2, draft

describe('categories', () => {
  test('only admins write; names are unique regardless of case', async () => {
    assert.equal((await learner.post('/categories', { label: 'Nope' })).status, 403)
...
describe('videos', () => {
  test('upload needs a real video file', async () => {
    ...
    assert.equal((await learner.upload('/videos', videoForm({ title: 'Nope' }))).status, 403)
  ...
  test('admin uploads with and without a poster; the API hides storage internals', async () => {
    ...
    assert.equal(lesson1.order, 1)
    assert.equal(lesson1.durationSec, 134)
    assert.equal(lesson1.videoUrl, `/api/v1/videos/${lesson1.id}/file`)
    assert.equal(lesson1.poster, `/api/v1/videos/${lesson1.id}/poster`)
    assert.equal(lesson1.videoFile, undefined)
    assert.equal(lesson1.size, webm().length)
    const b = await admin.upload('/videos', videoForm({ title: 'A warm introduction', categoryId: daily.id }, { video: mp4() }))
    lesson2 = b.body.data
    assert.equal(lesson2.order, 2)
    assert.equal(lesson2.poster, null)
    assert.equal(lesson2.status, 'published')
  ...
  test('visibility: learners and visitors only see published, categorised lessons', async () => {
    ...
    assert.deepEqual(titles(await learner.get('/videos')), ['A warm introduction', 'Nice to meet you'])
    assert.deepEqual(titles(await new Client(base).get('/videos')), ['A warm introduction', 'Nice to meet you'])
    assert.equal((await learner.get(`/videos/${draft.id}`)).status, 404)
  ...
  test('the file streams with Range support, and needs a session', async () => {
    assert.equal((await new Client(base).get(`/videos/${lesson1.id}/file`)).status, 401)
    const full = await learner.get(`/videos/${lesson1.id}/file`, { raw: true })
    assert.equal(full.status, 200)
    assert.equal(full.headers.get('accept-ranges'), 'bytes')
    assert.equal(full.headers.get('content-type'), 'video/webm')
    assert.deepEqual(full.body, webm())
    const part = await learner.get(`/videos/${lesson1.id}/file`, { raw: true, headers: { range: 'bytes=10-19' } })
    assert.equal(part.status, 206)
    assert.equal(part.headers.get('content-range'), `bytes 10-19/${webm().length}`)
    assert.deepEqual(part.body, webm().subarray(10, 20))
    const tail = await learner.get(`/videos/${lesson1.id}/file`, { raw: true, headers: { range: 'bytes=-5' } })
    assert.deepEqual(tail.body, webm().subarray(-5))
    assert.equal((await learner.get(`/videos/${lesson1.id}/file`, { raw: true, headers: { range: 'bytes=99999-' } })).status, 416)
    assert.equal((await learner.get(`/videos/${draft.id}/file`)).status, 404)
    const mp4res = await learner.get(`/videos/${lesson2.id}/file`, { raw: true })
    assert.equal(mp4res.headers.get('content-type'), 'video/mp4')
  })

  test('posters are served as images', async () => {
    const res = await new Client(base).get(`/videos/${lesson1.id}/poster`, { raw: true })
    assert.equal(res.status, 200)
    assert.equal(res.headers.get('content-type'), 'image/jpeg')
    assert.equal((await new Client(base).get(`/videos/${lesson2.id}/poster`)).status, 404)
  })

  test('editing: moving categories appends to the end of that path', async () => {
    const moved = await admin.patch(`/videos/${lesson2.id}`, { categoryId: work.id })
    assert.equal(moved.body.data.categoryId, work.id)
    assert.equal(moved.body.data.order, 1)
    const back = await admin.patch(`/videos/${lesson2.id}`, { categoryId: daily.id })
    assert.ok(back.body.data.order > draft.order, 'moved video goes after everything already in the path')
    assert.equal((await admin.patch(`/videos/${lesson2.id}`, { status: 'archived' })).status, 422)
    assert.equal((await admin.patch(`/videos/${lesson2.id}`, { categoryId: '64b7f0f0f0f0f0f0f0f0f0f0' })).status, 422)
    ...
    assert.equal((await learner.patch(`/videos/${lesson2.id}`, { title: 'hax' })).status, 403)
  })
})

let submission

describe('submissions and the lock rule', () => {
  const good = () => ({ videoId: lesson1.id, trimStart: '0', trimEnd: '2.5', duration: '3', mirrored: 'true' })
  ...
  test('input is checked before anything is stored', async () => {
    ...
    assert.equal((await learner.upload('/submissions', recordingForm(good(), { file: null }))).status, 422)
    assert.equal((await learner.upload('/submissions', recordingForm(good(), { file: Buffer.from('plain text pretending to be video') }))).status, 422)
    assert.equal((await learner.upload('/submissions', recordingForm({ ...good(), trimStart: '2', trimEnd: '1' }))).status, 422)
    assert.equal((await learner.upload('/submissions', recordingForm({ ...good(), trimEnd: '30' }))).status, 422)
    assert.equal((await learner.upload('/submissions', recordingForm({ ...good(), videoId: 'nope' }))).status, 422)
    assert.equal((await learner.upload('/submissions', recordingForm({ ...good(), videoId: draft.id }))).status, 404)
    assert.equal(filesIn('recordings').length, before)
    assert.deepEqual(filesIn('tmp'), [])
  })

  test('a valid submission is stored and listed', async () => {
    const res = await learner.upload('/submissions', recordingForm(good()))
    assert.equal(res.status, 201, JSON.stringify(res.body))
    submission = res.body.data
    assert.equal(submission.videoId, lesson1.id)
    ...
    const mine = await learner.get('/submissions')
  ...
  test('LOCK: a second submission for the same lesson is refused and stores nothing', async () => {
    const again = await learner.upload('/submissions', recordingForm(good(), { file: mp4() }))
    assert.equal(again.status, 409)
    assert.match(again.body.error.message, /locked/i)
    assert.equal(filesIn('recordings').length, 1)
    assert.deepEqual(filesIn('tmp'), [])
  })

  test('LOCK: concurrent double-submits produce exactly one submission', async () => {
    const results = await Promise.all([1, 2, 3].map(() => other.upload('/submissions', recordingForm({ ...good(), videoId: lesson2.id }))))
  ...
  test('LOCK: there is no way to edit or delete a submission, and learners cannot watch it back', async () => {
    assert.equal((await learner.patch(`/submissions/${submission.id}`, { trimEnd: 1 })).status, 404)
    assert.equal((await learner.delete(`/submissions/${submission.id}`)).status, 404)
    assert.equal((await learner.get(`/submissions/${submission.id}/recording`)).status, 403)
  ...
  test('deleting a base video keeps learners\' submissions on record', async () => {
    const before = filesIn('videos').length
    assert.equal((await admin.delete(`/videos/${lesson2.id}`)).status, 204)
    assert.equal(filesIn('videos').length, before - 1, 'the base video file is removed from storage')
    assert.equal((await admin.get(`/videos/${lesson2.id}`)).status, 404)
    assert.equal((await other.get('/submissions')).body.data.length, 1)
  })
})
...
describe('admin stats and contact', () => {
  test('stats add up', async () => {
    assert.equal((await learner.get('/admin/stats')).status, 403)
    const res = await admin.get('/admin/stats?days=7')
    assert.equal(res.status, 200)
    const { totals, activity, byCategory, recent } = res.body.data
    assert.equal(totals.learners, 3) // sara, omar, eve
    assert.equal(totals.submissions, 2)
    assert.equal(totals.categories, 2)
    assert.equal(totals.videos, 3)
    assert.equal(totals.publishedVideos, 2) // lesson 1 and the unassigned one; the draft is not counted
    assert.equal(activity.length, 7)
    assert.equal(activity.reduce((sum, d) => sum + d.count, 0), 2)
    assert.equal(activity.at(-1).date, new Date().toISOString().slice(0, 10))
    // Omar's lesson was deleted earlier, so it counts towards totals and activity but no longer belongs to a category.
    assert.equal(byCategory.find((c) => c.categoryId === daily.id).count, 1)
    assert.equal(byCategory.find((c) => c.categoryId === work.id).count, 0)
    assert.equal(recent.length, 2)
    assert.ok(['Sara Ahmed', 'Omar Khan'].includes(recent[0].learner.name))
    assert.equal(recent.find((r) => r.video === null)?.category, null) // the deleted lesson
  ...
  test('deleting a category unassigns its videos instead of deleting them', async () => {
    ...
    const kept = await admin.get(`/videos/${lesson1.id}`)
    assert.equal(kept.status, 200)
    assert.equal(kept.body.data.categoryId, null)
    assert.equal((await learner.get(`/videos/${lesson1.id}`)).status, 404) // hidden from learners now
  })
  ...
  test('contact messages: public to send, admin to read', async () => {
    ...
    assert.equal((await learner.get('/admin/messages')).status, 403)
  })
